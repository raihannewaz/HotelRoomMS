...
namespace Common.Core.FileUpload;
...
public class LocalFileStorageService : IFileStorageService
{
    ...
    where T : class
    {
        ...
        if (streamData.Length > 0)
        {
            ...
            {
                FileType.Image => Path.Combine("wwwroot", "Images", folder),
                _ => Path.Combine("wwwroot", "Others", folder),
            };
            ...
            using var image = await Image.LoadAsync(streamData, cancellationToken);


            image.Mutate(x => x.Resize(new ResizeOptions
            {
                Mode = ResizeMode.Max,
                Size = new Size(1200, 628)
            }));

            var extension = request.Extension.ToLower();

            if (extension == ".jpg" || extension == ".jpeg")
            {
                var jpegEncoder = new JpegEncoder { Quality = 90 };
                await image.SaveAsync(fullPath, jpegEncoder, cancellationToken);
            }
            else if (extension == ".png")
            {
                var pngEncoder = new PngEncoder { CompressionLevel = PngCompressionLevel.Level6 };
                await image.SaveAsync(fullPath, pngEncoder, cancellationToken);
            }
            ...
            return dbPath.Replace("\\", "/").Replace("wwwroot/", "");
