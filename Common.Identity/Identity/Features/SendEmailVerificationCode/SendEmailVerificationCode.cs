...
namespace Common.Identity.Identity.Features.SendEmailVerificationCode;

public record SendEmailVerificationCode(string Email) : IRequest;

internal class SendEmailVerificationCodeHandler : IRequestHandler<SendEmailVerificationCode>
{
    ...
    public async Task<Unit> Handle(SendEmailVerificationCode request, CancellationToken cancellationToken)
    {
        ...
        bool isExists = await _context.Set<EmailVerificationCode>()
            .AnyAsync(
                evc => evc.Email == request.Email && evc.SentAt.AddMinutes(5) > DateTimeConversion.UTCToBST(), cancellationToken);
        ...
        {
            Code = verificationCode, Email = request.Email, SentAt = DateTimeConversion.UTCToBST()
        };
